...
# Lazy, on-demand imports (HuggingFace or ESMC)
try:
    from transformers import AutoTokenizer, AutoModel  # for esm2 / esm1b series
except Exception:
    AutoTokenizer = None  # not needed if only ESMC is used
    AutoModel = None
...
# =============================================
# Map of model short names to actual loading configs
# =============================================
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate LMDB embeddings (CLS & middle tokens) for protein sequences using ESM / ESMC models"
    )
...
def resolve_model(short_name: str) -> Tuple[str, str]:
    """Parse the model short name into backend type and internal load name.
    Returns (backend, internal_name)
    backend in {"hf", "esmc"}
    """
    sn = short_name.lower()
    if sn in HF_MODEL_NAME_MAP:
        return "hf", HF_MODEL_NAME_MAP[sn]
    if sn in ESMC_MODEL_NAME_MAP:
        return "esmc", ESMC_MODEL_NAME_MAP[sn]
    raise ValueError(f"未支持的模型短名称: {short_name}")


def load_model(backend: str, internal_name: str, short_name: str, device: torch.device):
    """Load model and tokenizer/sequence tokenizer according to backend.
    Returns (model_or_client, tokenizer_obj, meta_dict)
    meta_dict may include hidden_size (if directly available)
    """
    if backend == "hf":
        if AutoTokenizer is None or AutoModel is None:
            raise RuntimeError("Transformers is not installed; cannot load HuggingFace models.")
        tokenizer = AutoTokenizer.from_pretrained(internal_name)
        model = AutoModel.from_pretrained(internal_name, torch_dtype=torch.bfloat16).to(device)
        model.eval()
        hidden_size = getattr(model.config, "hidden_size", None)
        print(f"Loaded HF model: {internal_name}, hidden_size={hidden_size}")
        return model, tokenizer, {"hidden_size": hidden_size}

    if backend == "esmc":
        if not ESMC_AVAILABLE:
            raise RuntimeError("ESMC-related libraries not detected; please install first.")
        client = ESMC.from_pretrained(internal_name, device=device)
        tokenizer = EsmSequenceTokenizer()
        # If local weights exist, try loading them
...
def get_embeddings(backend: str, model_or_client, tokenizer, device, sequence: str) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
    ...
    backend == 'hf': use transformers outputs
    backend == 'esmc': use client.logits(...).embeddings[-1]
        ...
        if backend == "hf":
            with torch.no_grad():
                ...
                with torch.autocast(device_type=device.type, dtype=torch.bfloat16):
                    outputs = model_or_client(**inputs)
                emb = outputs.last_hidden_state  # [1, L, H]
                cls_embedding = emb[0, 0].float().cpu().numpy()
...
def main():
    ...
    if args.source_lmdb:
        ...
        model_or_client, tokenizer, meta = load_model(backend, internal_name, args.model, device)
